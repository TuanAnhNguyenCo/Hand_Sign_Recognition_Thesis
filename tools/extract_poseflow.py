...
def main():
    input_dir_ = '/mnt/disk2/anhnct/Hand-Sign-Recognition/data/VN_SIGN/poses'
    ...
    for input_dir in input_dirs:
        ...
        output_dir = input_dir.replace('poses', 'poseflow')
        if os.path.exists(output_dir):
            continue
        os.makedirs(output_dir, exist_ok=True)

        kp_files = sorted(glob.glob(os.path.join(input_dir, '*.json')))[:-2]
